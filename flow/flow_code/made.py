...
class RealNVP(nn.Module):
    ...
    def sample(self, state_action, y=None):
        """
        根据给定的 (state, action) 对生成对应的下一个状态的样本。
        """
        # 假设 state_action 是 (num_samples, state_action_dim) 维度的张量
        # 这里的逆向过程是将输入 (state, action) 变换为下一个状态
        next_state, _ = self.inverse(state_action, y)
        return next_state
# --------------------
# Train and evaluate
# --------------------
