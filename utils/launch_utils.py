    ...
    for key, item in d.items():
        if safe_json(item):
            ...
            if isinstance(item, dict):
                ...
                new_d[key] = str(item)  # if list or tuple, to str
    return new_d
...
def create_log_dir(
    ...
    if base_log_dir[0] == '~':
        # expand ~ as user dir: ~/mbrl to /home/dx/mbrl
...
def setup_logger(
    ...
    if variant is not None:  # config
        # print config info to console or log_file
        logger.log("Variant:")
        # log config info of json format, indent=2 indent 2 characters
    ...
    # log out info add to logger, so easy to write log info
...
def try_eval(v):
    try:
        return eval(v)  # compute string: '1+9'
    except:
        return v  # capture except, such as 'abc + 1'

def _set_config_by_k_v(config, k, v):
    from mbrl.algorithms.utils import _visit_all_items
    v = try_eval(v)  # compute number from json file, such as number is '2*8' .
    keys = k.split('.')
    find = False
    if len(keys) == 2: 
...
def _run_experiments(config_path, cmd_config):
    ...
    if 'exp_prefix' not in config['experiment']:
        # delete path and only save file name:
        # ./osth/base.py ==> base.py
...
def run_single_experiment(config):
    ...
    # get dynamically algorithm under mbrl dir
    algo = get_item('algorithm', algo_class, algo_kwargs)
    
    algo.to(ptu.device)  # instance OurMBAlgorithm()
    algo.train()  # use RLAlgorithm train function


def parse_cmd():
    p = argparse.ArgumentParser()
    p.add_argument('config_file', type=str)  # python train.py ant.json ,除去python和执行了的train.py 剩下的文件就是被读取配置文件 必须填写
    p.add_argument('--env_name', type=str)  # -- 可以选择填写
